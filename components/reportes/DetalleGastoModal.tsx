           ...
           <section className="bg-slate-50/50 rounded-2xl border border-slate-200 overflow-hidden">
             ...
             <div className="flex items-center justify-between px-6 py-4 bg-white border-t border-slate-100">
               <span className="text-xs font-bold text-slate-500">
                 Página {Math.min(pagina, Math.max(1, Math.ceil(gastos.length / pageSize))) || 1} de {Math.max(1, Math.ceil(gastos.length / pageSize))}
               </span>
               <div className="flex items-center gap-2">
                 <button
                   onClick={() => setPagina(p => Math.max(1, p - 1))}
                   disabled={pagina === 1 || gastos.length === 0}
                   className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${(pagina === 1 || gastos.length === 0) ? 'text-slate-300 border-slate-200 bg-slate-50 cursor-not-allowed' : 'text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                 >
                   Anterior
                 </button>
                 <button
                   onClick={() => setPagina(p => Math.min(Math.ceil(gastos.length / pageSize), p + 1))}
                   disabled={pagina >= Math.ceil(gastos.length / pageSize) || gastos.length === 0}
                   className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${(pagina >= Math.ceil(gastos.length / pageSize) || gastos.length === 0) ? 'text-slate-300 border-slate-200 bg-slate-50 cursor-not-allowed' : 'text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                 >
                   Siguiente
                 </button>
               </div>
