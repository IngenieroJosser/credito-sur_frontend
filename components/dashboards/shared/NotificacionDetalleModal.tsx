        ...
        >
          ...
          <div className="p-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
            ...
            <div className="space-y-6">
              ...
              {/* ── Bloque especifico: Prorroga y Gestion de Cuenta Vencida ── */}
              {(approvalType === 'PRORROGA_PAGO' || (safeMeta?.tipoAprobacion === 'PRORROGA_PAGO') || safeMeta?.tipo === 'GESTION_VENCIDA' || safeMeta?.tipo === 'ASIGNAR_MORA') && (() => {
                const d = editedDetails
                const decision = d?.decision || safeMeta?.decision || 'PRORROGAR'
                const DECISION_COLORS: Record<string, { bg: string; border: string; text: string; badge: string }> = {
                  PRORROGAR:    { bg: 'bg-blue-50/60',  border: 'border-blue-200',  text: 'text-blue-700',  badge: 'bg-blue-100 text-blue-800' },
                  CASTIGAR:     { bg: 'bg-rose-50/60',  border: 'border-rose-200',  text: 'text-rose-700',  badge: 'bg-rose-100 text-rose-800' },
                  JURIDICO:     { bg: 'bg-slate-50/60', border: 'border-slate-200', text: 'text-slate-700', badge: 'bg-slate-100 text-slate-800' },
                  ASIGNAR_MORA: { bg: 'bg-amber-50/60', border: 'border-amber-200', text: 'text-amber-700', badge: 'bg-amber-100 text-amber-800' },
                }
                ...
                const colores = DECISION_COLORS[decision] || DECISION_COLORS.PRORROGAR
                const nuevaFecha = d?.nuevaFechaVencimiento || safeMeta?.nuevaFechaVencimiento
                const fechaOrig = d?.fechaVencimientoOriginal || safeMeta?.fechaVencimientoOriginal
                const diasGracia = d?.diasGracia || safeMeta?.diasGracia
                const montoInteres = Number(d?.montoInteres || safeMeta?.montoInteres || d?.montoInteres || 0)
                const saldoPendiente = Number(d?.saldoPendiente || safeMeta?.saldoPendiente || 0)
                const comentarios = d?.comentarios || safeMeta?.comentarios || d?.razon
                const clienteNombre = d?.cliente || d?.clienteNombre || safeMeta?.cliente || safeMeta?.clienteNombre
                const numeroPrestamo = d?.numeroPrestamo || safeMeta?.numeroPrestamo
                const gestionadoPor = d?.gestionadoPor || safeMeta?.gestionadoPor || d?.asignadoPor || safeMeta?.asignadoPor || solicitante
                ...
                return (
                  <div className={`rounded-2xl border p-5 space-y-4 ${colores.bg} ${colores.border}`}>
                    {/* Tipo de decision */}
                    <div className="flex items-center justify-between">
                      <p className={`text-[9px] font-black uppercase tracking-widest ${colores.text}`}>Detalle de la Solicitud</p>
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black ${colores.badge}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="col-span-2 bg-white/70 rounded-xl p-3 border border-white">
                      ...
                      {numeroPrestamo && (
                        <div className="bg-white/70 rounded-xl p-3 border border-white">
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">No. Prestamo</p>
                          <p className="text-sm font-black text-slate-900">{numeroPrestamo}</p>
                        </div>
                      )}
                      {saldoPendiente > 0 && (
                        <div className="bg-white/70 rounded-xl p-3 border border-white">
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Saldo Pendiente</p>
                          <p className="text-sm font-black text-rose-700">{formatCurrency(saldoPendiente)}</p>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {diasGracia !== undefined && diasGracia !== null && (
                          <div className="bg-white/70 rounded-xl p-3 border border-white text-center">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Dias de Gracia</p>
                            <p className="text-2xl font-black text-blue-700">{diasGracia}</p>
                            <p className="text-[9px] text-slate-400 font-bold">dias</p>
                          </div>
                        )}
                        {fechaOrig && (
                          <div className="bg-white/70 rounded-xl p-3 border border-white text-center">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Fecha Orig.</p>
                            <p className="text-xs font-black text-slate-700">{fmt(fechaOrig)}</p>
                          </div>
                        )}
                        {nuevaFecha && (
                          <div className="bg-blue-100/80 rounded-xl p-3 border border-blue-200 text-center">
                            <p className="text-[9px] font-black text-blue-600 uppercase tracking-widest mb-0.5">Nueva Fecha</p>
                            <p className="text-xs font-black text-blue-900">{fmt(nuevaFecha)}</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {/* Interes adicional */}
                    {montoInteres > 0 && (
                      <div className="bg-white/70 rounded-xl p-3 border border-white flex items-center justify-between">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Interes / Mora adicional</p>
                    ...
                    {comentarios && (
                      <div className="bg-white/70 rounded-xl p-3 border border-white">
                        ...
                        <p className="text-xs text-slate-700 font-medium italic leading-relaxed">"{comentarios}"</p>
                      </div>
                    )}
                    ...
                    {gestionadoPor && (
                      <div className="flex items-center gap-2 pt-1 border-t border-white/50">
                        ...
                        <p className="text-[10px] font-bold text-slate-500">Solicitado por <span className="text-slate-800 font-black">{gestionadoPor}</span></p>
                      </div>
                    )}
                  </div>
                )
