...
import { Calendar, CheckCircle2, ChevronLeft, Clock, Eye, EyeOff, FileText, Lock, Phone, Shield, User, X } from 'lucide-react'
...
const PerfilUsuarioPage = () => {
  ...
  const [volverRuta, setVolverRuta] = useState('/supervisor')
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (user.rol === 'COBRADOR') setVolverRuta('/cobranzas')
      else if (user.rol === 'SUPERVISOR') setVolverRuta('/supervisor')
      else setVolverRuta('/admin')
    } catch {
      // ignore
    }
  }, [])
        ...
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight">
              <span className="text-blue-600">Información personal </span>
              <span className="text-orange-500">y de acceso</span>
            </h1>
          </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm p-8 shadow-sm flex flex-col items-center text-center">
              ...
              <div className="w-full pt-6 border-t border-slate-100 space-y-4">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded text-xs">{userData.rol}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500 font-medium flex items-center gap-2">
          ...
          <div className="lg:col-span-2 space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm p-8 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Teléfono Móvil</label>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm p-8 shadow-sm">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                  ...
                  <button onClick={handleOpenPasswordModal} className="text-sm font-bold text-slate-900 hover:text-blue-600 transition-colors">
                    Actualizar
                  </button>
                </div>
          ...
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-full max-w-lg border border-slate-200 shadow-2xl p-8 transform scale-100 animate-in zoom-in-95 duration-200 relative">
              ...
              <h2 className="text-2xl font-bold text-slate-900 mb-2 tracking-tight">
                Cambiar <span className="font-light text-slate-500">Contraseña</span>
              </h2>
              ...
              <div className="space-y-5">
                ...
                <div className="pt-4 flex gap-3">
                  ...
                  <button className="flex-1 px-4 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-sm hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/20">
                    Guardar Cambios
                  </button>
                </div>
              </div>
