...
/**
 * Vista de Cuentas Vencidas para el Supervisor.
 * Reutiliza el mismo componente visual que Admin/SuperAdmin.
 * El Supervisor puede ver y exportar pero NO gestionar (prorrogar/castigar).
 * La prop `roles` en ProtectedPage evita bloqueos por permisos granulares.
 */
...
function CuentasVencidasContent() {
  ...
  const fetchCuentasVencidas = useCallback(async () => {
    try {
      ...
      const response = await vencidasService.obtenerCuentasVencidas({
  ...
  useRealtimeData(['pagos_actualizados', 'prestamos_actualizados', 'aprobaciones_actualizadas'], fetchCuentasVencidas)
      ...
      <div className="relative z-10 px-6 md:px-8 py-8 space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="min-w-0">
            <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700 mb-2 border border-slate-200">
              ...
              <span>Cuentas Vencidas — Contrato Expirado</span>
            </div>
            ...
            <p className="text-sm text-slate-500 max-w-2xl mt-1 font-medium">
              Créditos con contrato expirado que aún tienen saldo pendiente.
              <span className="text-slate-400 ml-2">({cuentasFiltradas.length} registros)</span>
            </p>
          </div>
          <button
            onClick={fetchCuentasVencidas}
            className="p-2 text-slate-400 hover:text-blue-600 bg-white border border-slate-200 rounded-xl shadow-sm transition-all self-start"
            title="Actualizar"
          >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {['TODOS', 'Reciente', 'Moderado', 'Grave', 'Crítico'].map(s => {
            ...
            return (
              <button key={s} onClick={() => setFiltroSeveridad(s)}
                className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black border transition-all',
                  isActive ? s === 'TODOS' ? 'bg-slate-900 text-white border-slate-900' : cn(badgeConfig[s] || '', 'shadow-sm')
        ...
        <div className="flex flex-col md:flex-row gap-3 items-center">
          ...
          <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
            <button onClick={() => setViewMode('list')} className={cn('p-2 rounded-lg transition-all', viewMode === 'list' ? 'bg-slate-100 text-primary shadow-sm' : 'text-slate-400')}>
              <List className="h-4 w-4" />
            </button>
            <button onClick={() => setViewMode('grid')} className={cn('p-2 rounded-lg transition-all', viewMode === 'grid' ? 'bg-slate-100 text-primary shadow-sm' : 'text-slate-400')}>
                    ...
                    return (
                      <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                        <td className="px-6 py-4">
                          <div className="font-bold text-slate-900 group-hover:text-primary transition-colors">{c.numeroPrestamo}</div>
                ...
                <div key={cuenta.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col group">
                  ...
                  <div className="p-5 flex-1 space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="min-w-0">
                        <div className="font-black text-slate-900 group-hover:text-primary transition-colors">{cuenta.numeroPrestamo}</div>
