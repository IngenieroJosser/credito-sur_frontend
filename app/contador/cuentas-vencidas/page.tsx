            ...
            <div className="relative z-10 px-6 md:px-8 py-8 space-y-8 text-slate-900">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between animate-in fade-in slide-in-from-top-4 duration-500">
                    <div>
                        <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600 mb-2 border border-slate-200">
                            ...
                            <span>Contabilidad / Cartera Castigada</span>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                        <ExportButton label="Exportar Reporte" onExportExcel={handleExportExcel} onExportPDF={handleExportPDF} />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-xs font-bold text-slate-400 uppercase">Cartera Castigable</p>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-xs font-bold text-slate-400 uppercase">Días Promedio Mora</p>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filtradas.map(c => (
                                <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <Link
                                            href={`/contador/cuentas-vencidas/${c.id}`}
                                            className="inline-flex items-center gap-1.5 px-4 py-2 bg-white border border-slate-200 text-slate-600 font-bold rounded-lg hover:bg-slate-50 hover:text-slate-800 transition-all text-xs shadow-sm hover:shadow-md"
                                        >
                                            <Archive className="h-3.5 w-3.5" />
                                            Procesar
                                        </Link>
...
export default CuentasVencidasContador
